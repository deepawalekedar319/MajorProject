...
def bicycle_view(request, data=None):
    ...
    if data==None:
        ...
        allCycles=Product.objects.filter(category='BC').filter(discounted_price__lte=10000)
    elif data=='above':
        allCycles=Product.objects.filter(category='BC').filter(discounted_price__gte=10000)
...
def trains_view(request, data=None):
    ...
    if data==None:
        ...
        allTrains=Product.objects.filter(category='TT').filter(discounted_price__lte=10000)
    elif data=='above':
        allTrains=Product.objects.filter(category='TT').filter(discounted_price__gte=10000)  
...
def dolls_view(request, data=None):
    ...
    if data==None:
        ...
        allDolls=Product.objects.filter(category='BT').filter(discounted_price__lte=10000)
    elif data=='above':
        allDolls=Product.objects.filter(category='BT').filter(discounted_price__gte=10000)  
    return render(request,'app/barbie.html',{'allCycles':allDolls})   

def food_counter(request):
